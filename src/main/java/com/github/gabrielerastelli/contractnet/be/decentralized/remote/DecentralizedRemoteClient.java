package com.github.gabrielerastelli.contractnet.be.decentralized.remote;
...
@AllArgsConstructor
public class DecentralizedRemoteClient {
    ...
    public TaskTuple readTask() throws TupleSpaceException, RemoteException {
        ITuple template = new Tuple().add(new Field().setValue("t")).add(new Field().setType(String.class))
                .add(new Field().setType(Integer.class));
        ITuple retrieved = space.inp(template);
        return retrieved == null ? null : new TaskTuple(retrieved.get(1).toString(),
                Integer.parseInt(retrieved.get(2).toString()));
    }
